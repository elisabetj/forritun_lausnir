...
def random_string_float():
    ...
    insert_pos = random.randint(1, len(base_float) - 3 if base_float[0] != '-' else len(base_float) - 2)  # Avoiding the last two digits (after the decimal) and consider negative sign
...
def random_string_integer(length):
    base_int = random_integer(1, 10**length - 1)  # This generates an integer with the given length
    if random.choice([True, False]):
        base_int = -base_int
    s = str(base_int)
...
def random_float_with_underscores(min_x, max_x, decimals):
    ...
    num_underscores = random.randint(1, len(base_float) // 2)  # Just a heuristic for the number of underscores.
...
if data_type == "string":
    ...
    a = random.choice([random_float_with_decimals(min_x, max_x, decimals), random_string(5)])
    b = random.choice([random_float_with_decimals(min_x, max_x, decimals), random_string(5)])
    print(a)
    print(b)
