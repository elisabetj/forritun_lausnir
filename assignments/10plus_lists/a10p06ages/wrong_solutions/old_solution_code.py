...
def gather_all_inputs() -> list:
    number_of_people = ask_for_integer("Number of people:\n")
    names_and_ages = gather_names_and_ages(number_of_people)
    return names_and_ages


def ask_for_integer(text) -> int:
...
def gather_names_and_ages(how_many: int) -> list:
    names_and_ages: list = []
    for person in range(1, how_many + 1):
        name: str = input(f"Name of person {person}:\n")
        age: int = ask_for_integer(f"Age of person {person}:\n")
