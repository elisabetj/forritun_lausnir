...
def get_median(numbers: List[float]) -> float | None:
    ...
    # The list is zero-indexed, i.e. 0, 1, ..., length -1
    if length % 2 == 1:
        assert middle_index == (length - 1) / 2
        # If length is odd, then middle_index is just that.

        # For example if length = 5, then the list is indexed 0, 1, 2, 3, 4,
        # and middle_index is 2, smack in the middle.
        return sorted_numbers[middle_index]
    else:
        assert middle_index == length / 2
        # But if length is even, then the median is the average of two numbers,
        # the ones on either side of (length-1)/2,
        # i.e. (length-2)/2 = length/2 - 1 on the left
        # and length/2 on the right.

        # For example, if length = 4, then the list is indexed 0, 1, 2, 3,
        # and middle_index is 4/2 = 2, and the middle elements are at indices 1 and 2.
        # Note that 3/2 = 1.5, and the middle elements are on either side of 1.5.
