...
print(
    f"Transactions for stock: {stock_symbol}",
    f"Bought {shares} shares @ {buy_price}",
    f"Paid {round(total_buy_price, DECIMAL_DIGITS)}",
    f"Commission when buying: {round(buy_commission, DECIMAL_DIGITS)}",
    f"Sold {shares} shares @ {sell_price}",
    f"Received {round(total_sell_price, DECIMAL_DIGITS)}",
    f"Commission when selling: {round(sell_commission, DECIMAL_DIGITS)}",
    f"Profit or loss: {round(profit_loss, DECIMAL_DIGITS)}",
    sep="\n",
)
